        ...
        <section className='container commonMT'>
            <div className="bg-[#181a27c9] border-2 border-[#545a686e] overflow-hidden rounded-[16px]">
                <div className='flexColCenter commonTextGap primaryBg px-2'>
                    <div className='py-2'>
                        <Image src={img} height={0} width={0} alt='bunles' className='w-auto h-auto' />
                    </div>
                    <h5 className='text-xl md:text-3xl lg:text-4xl/[50px] font-[500] [&_span]:primaryColor !font-roboto text-center lg:w-[50%]'>Check Out Everything You'll Get When You Grab the Bundle Today!</h5>
                </div>
                <div className='flex flex-col !justify-start commonTextGap px-1 py-4 md:p-3 lg:p-4'>
                    {
                        list.map((list: grabBundlesDataType) => {
                            return <div className='flexCenter !justify-between pb-4 border-[#545a686e] border-b-[2px] first:pt-2' key={list.id}>
                                <div className='flexCenter !justify-start gap-2 md:gap-3 lg:gap-4 max-399:w-[194px] w-[70%] xl:w-max' >
                                    <span className='primaryColor text-sm md:text-base'> <FaCheckCircle /></span>
                                    <span className='font-semibold temax-399:xt-xs/5 sm:text-lg'>{list.text}</span>
                                </div>
                    ...
                    <div className='flex flex-col !justify-start commonTextGap p-2 md:p-4 '>
                        <div className='flexColCenter font-bold commonTextGap !justify-between pb-4 md:py-6 border-[#545a686e] border-b-[2px]'>
                            <span className='text-xl md:text-3xl lg:text-4xl text-[#ffc66bb2] overflow-hidden after:-rotate-[2.39deg] relative after:content-[""] after:absolute after:bottom-4 after:left-0 after:bg-[#d3d3d3] after:w-[110%] after:h-[3px]'>Total value: $7788</span>
                            <span className='text-xl md:text-3xl lg:text-4xl text-[#ffc66b]'>Normal Price : $1299</span>
                        </div>
                    </div>
                    <div className='primaryBg text-center rounded-[8px] p-2 md:p-4'>
                        <h6 className='text-[#ffffff99] text-xl md:text-3xl lg:text-4xl'>If You Buy Now <span className='text-white font-bold'>$699</span></h6>
                    </div>
                    <CommonBtnComp />
                </div>
            </div>
