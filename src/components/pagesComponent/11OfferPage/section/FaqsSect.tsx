    ...
    return (
        <section className='container commonMB commonMT space-y-8 md:space-y-12 lg:space-y-16'>
            <div className="flexColCenter mx-auto text-center commonTextGap">
                <h6 className='text-xl md:text-3xl lg:text-4xl/[50px] font-[500] [&_span]:primaryColor !font-roboto text-center lg:text-start'>Don't Just Take Our Word For It - Hear From <span>Our Satisfied Customers!</span></h6>
            </div>
            <div>
                <Accordion type="single" collapsible className="w-full space-y-6">
                    {
                        data.map((item: faqsDataTypes) => {
                            return <AccordionItem value={item.id.toString()} key={item.id} className='bg-[#316be71a] border-none rounded-[8px] p-4'>
                                <AccordionTrigger className='text-base data-[state=open]:border-b pb-2 [&>svg]:w-[24px] [&>svg]:h-[24px] [&>svg]:primaryColor shadow-[0_1px_5px_0_#0000001f]'>{item.question}</AccordionTrigger>
