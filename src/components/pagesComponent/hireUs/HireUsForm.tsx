...
const HireUsForm = () => {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (validateForm()) {
      try {
        ...
        try {
          ...
          if (response.ok && responseData && responseData.error === false) {
            ...
            if (responseData.details && responseData.details.includes("405")) {
              toast.error(
                "The server doesn't support this request method. We'll use the email option instead."
              );
            } else if (responseData.status === 404) {
              toast.error(
                "The API endpoint couldn't be found. We'll use the email option instead."
              );
            } else {
              toast.error(
          ...
          toast.error(
            "We couldn't connect to our server. We'll use the email option instead."
          );
        ...
        toast.error(
          "We're experiencing technical difficulties. Please use the email option."
        );
  ...
  const handleEmailFallback = (data: FormData) => {
    ...
    try {
      ...
      const mailtoLink = `mailto:${supportEmail}?subject=Hire%20Us%20Request%20from%20${encodeURIComponent(
        data.name
      ...
      toast.success(
        "We've opened an email window for you to send your request directly to our team. Please send the email to complete your request."
      );

      // Show a message explaining what to do if the email client doesn't open
      toast(
        `If your email client didn't open, please send an email to ${supportEmail} with your details.`,
        {
