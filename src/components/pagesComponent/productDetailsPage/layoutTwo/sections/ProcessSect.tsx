    ...
    return (
        <section className='container commonMT space-y-12 md:space-y-16 lg:space-y-20'>
            <div className='flexColCenter commonTextGap md:w-[60%] lg:w-[55%] m-auto text-center'>
                <h2 className='sectionTitle'>Process From Purchase to Publication in 3 Simple Steps</h2>
                <p className='sectionPara'>
                    Get the app from purchase to publication in 3 simple s	teps. It's quick, easy, and hassle-free
                </p>
            </div>
                    ...
                    cardsdata.map((item: servicesProcessDataTypes, index: number): React.ReactNode => (
                        <div key={index} className="flex flex-col items-center text-center mb-12 md:mb-0 max-w-xs">
                            <div className="relative">
                                <div className="">
                                    <Image src={item.img} height={0} width={0} alt='process-icon' className='w-full h-full' />
                                </div>
                            </div>
