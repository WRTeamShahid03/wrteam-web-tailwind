...
const MarqueSect: React.FC = () => {
  const slug = usePathname();
  const [data, setData] = useState<marqueTestimonialDataType[]>([]);

  useEffect(() => {
    // Define arrays inside useEffect to avoid dependency issues
    const eClassify = [
      {
        id: 0,
        ...
        <div className="flexCenter gap-6 py-6">
          {data?.map((data) => {
            return (
              <div
                key={data.id}
                className="flexCenter gap-4 shadow-[0_2px_4px_0_#0000001f] p-3 rounded-[8px]"
              >
                <Image
                  src={img}
                  height={0}
