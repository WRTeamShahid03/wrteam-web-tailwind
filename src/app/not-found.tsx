...
const Custom404 = () => {
  ...
  const salePages = [
    '/cyber-week-sale', '/steals-of-the-week-sale', '/extended-license-sale-25', '/new-year-sale',
    '/freedom-sale', '/mid-year-sale', '/diwali-sale', '/cyber-sale'
  ];
  const isSalePage = salePages.some((salePath) => pathname.startsWith(salePath));
  ...
  useEffect(() => {
    ...
    if (!redirected && !isSalePage && typeof window !== 'undefined' && window.history.length > 1) {
      setCanGoBack(true);
    }
...
const SaleEndedView = () => {
  ...
  return (
    <div className="flex min-h-screen flex-col justify-between bg-[#F7F9FC] text-[#0A2D75] items-center">
      <div className="mx-auto flex w-full max-w-6xl flex-col items-center gap-8 px-6 py-10 lg:flex-row lg:py-14">
        <div className="relative flex w-full justify-center lg:w-1/2">
          <Image
            src={saleEndImg}
            alt="Sale ended illustration"
            className="relative z-[1] h-auto w-full max-w-[400px]"
            priority
          />
        </div>
        <div className="flex w-full flex-col gap-4 lg:w-1/2">
          <span className="w-max rounded bg-[#E5F0FF] px-3 py-1 text-xs font-semibold uppercase tracking-wide text-[#0A2D75]">Sale End</span>
          <h1 className="text-3xl font-extrabold leading-[1.1] sm:text-4xl">That Deal Has Ended… But the Good Stuff Hasn’t</h1>
          <p className="text-sm leading-relaxed text-[#4B5563] sm:text-base">
            Get early access to WRTeam updates, product launches, discounts, and important notifications,
            only on our <span className="font-semibold text-[#0A2D75]">WhatsApp channel</span>.
          </p>
          <Link
            href={whatsappChannelUrl}
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex w-max items-center gap-2 rounded-md bg-[#1E73F0] px-4 py-2 text-sm font-semibold text-white shadow-sm transition hover:bg-[#165ac3] focus:outline-none focus:ring-2 focus:ring-[#1E73F0] focus:ring-offset-2"
      ...
      <div className="bg-[#1E73F0] px-4 py-3 text-sm font-semibold text-white">
        <div className="flex flex-wrap items-center justify-center gap-4 sm:gap-6">
          {Array.from({ length: 10 }).map((_, idx) => (<span key={`sale-end-${idx}`}>Sale End</span>))}
        </div>
      </div>
    </div>
  );
